      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4">
          ...
          <Link to="/" className="flex items-center gap-3" aria-label="الصفحة الرئيسية">
            {settings?.logoUrl ? (
                <img src={settings.logoUrl} alt={settings?.siteName || "وصفاتي"} className="h-[52px] md:h-[58px] lg:h-[64px] w-auto object-contain" />
            ) : (
          ...
          <div className="hidden md:flex items-center gap-6">
            <form onSubmit={handleSearch} className="relative hidden lg:block">
              ...
              />
              <button type="submit" className="absolute right-3 top-2.5 text-slate-400 hover:text-orange-600 transition-colors">
          ...
          <div className="md:hidden flex items-center gap-4">
            <Link to="/search" className="text-slate-400 hover:text-slate-600">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white border-t border-slate-100 p-4 space-y-4 shadow-lg absolute inset-x-0 z-50 max-h-[calc(100vh-4rem)] overflow-y-auto">
          <form onSubmit={(e) => { handleSearch(e); setIsMobileMenuOpen(false); }} className="relative mb-4">
            ...
            />
            <button type="submit" className="absolute right-3 top-3 text-slate-400 hover:text-orange-600 transition-colors">
              <Search className="w-5 h-5" />
            </button>
          </form>
